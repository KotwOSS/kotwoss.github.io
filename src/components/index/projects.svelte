...
<main style:position={pin ? "fixed" : "relative"}>
    <h1 class:show={triggers.trigger(0, anim > 0)}>
        Projects
    </h1>
    <p class:show={triggers.trigger(1, anim > 0.2)}>
        You can find our projects on here
    </p>
    <div class="buttons">
        <button class:show={triggers.trigger(2, anim > 0.4)} on:click={gotoProjects}>
            Projects
        </button>
        <button class:show={triggers.trigger(3, anim > 0.6)} on:click={gotoGithub}>
            Github
        </button>
    </div>
</main>
